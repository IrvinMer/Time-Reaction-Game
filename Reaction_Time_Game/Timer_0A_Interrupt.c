...
void Timer_0A_Interrupt_Init(void (*task)(void)) {
  ...
  // Clear any pending Timer 0A timeout interrupt by writing 1 to TATOICR bit
  TIMER0->ICR |= 0x01;
  ...
  // Set the priority level to 2 for the Timer 0A interrupt
  ...
  NVIC->IPR[4] = (NVIC->IPR[4] & 0x00FFFFFF) | (2 << 29);
