 ...
 * @file Timer_0A_Interrupt.c
 <light>* @brief Source file for the Timer_0A_Interrupt driver.</light>
 *
 * This file contains the function definitions for the Timer_0A_Interrupt
 driver.
 ...
 * for the Timers lab. This driver assumes that the system clock's frequency is
 50 MHz.
 *
 * Refer to Table 2-9 (Interrupts) on pages 104 - 106 from the TM4C123G
 Microcontroller Datasheet
 * to view the Vector Number, Interrupt Request (IRQ) Number, and the Vector
 Address
 * for each peripheral.
 *
 * @Irvin
...
void Timer_0A_Interrupt_Init(void (*task)(void)) {
  ...
  // Set the R0 bit (Bit 0) in the RCGCTIMER register to enable the clock for
  // Timer 0A
  SYSCTL->RCGCTIMER |= 0x01;

  // Clear the TAEN bit (Bit 0) of the GPTMCTL register to disable Timer 0A
  TIMER0->CTL &= ~0x01;

  // Set the bits of the GPTMCFG field (Bits 2 to 0) in the GPTMCFG register
  // 0x04 = Select the 16-bit timer configuration
  TIMER0->CFG |= 0x04;
  ...
  // Timer 0A Resolution: 1 (1 us * 1000) = 1 ms
  TIMER0->TAILR = (1000 - 1);
  ...
  // Enable the Timer 0A interrupt by setting the TATOIM bit (Bit 0) in the
  // GPTMIMR register
  TIMER0->IMR |= 0x01;

  // Set the priority level to 1 for the Timer 0A interrupt
  // In the Interrupt 16–19 Priority (PRI4) register,
  // the INTA field (Bits 15 to 13) corresponds to Interrupt Request (IRQ) 19
...
void TIMER0A_Handler(void) {
  ...
  if (TIMER0->MIS & 0x01) {
    // Excute the user-defined function
    (*Timer_0A_Task)();
